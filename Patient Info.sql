...
SELECT DISTINCT t1.pat_id,
				...
				t2.death_tm,
				t3.HOSP_ADMSN_TIME,
				t3.HOSP_DISCHRG_TIME, 
				DATEDIFF(year, t1.birth_date, t3.HOSP_ADMSN_TIME) as AgeNBR, 
				t3.height,
				t3.weight,
				t1.sex_c, 
...
FROM EDW_SOURCE_ZONE_CLARITY.Clinical.Patient t1
				LEFT JOIN EDW_SOURCE_ZONE_CLARITY.Clinical.Patient_2 t2 on t1.pat_id = t2.pat_id
                RIGHT JOIN EDW_SOURCE_ZONE_CLARITY.CLINICAL.PAT_ENC t3 on (t1.pat_id = t2.pat_id)
